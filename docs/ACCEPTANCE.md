...
| Dock 与菜单栏 | 通过 | 包内 `LSUIElement=false`，显示 Dock 图标并保留菜单栏；区域/全屏截图与录屏、导入、最近素材直接可用；包验证脚本会拒绝错误隐藏 Dock 的构建 |
| 主题对比度 | 通过 | 默认跟随系统；亮色模式使用深绿色强调色、语义背景与分隔线，选中按钮保持高对比文字 |
